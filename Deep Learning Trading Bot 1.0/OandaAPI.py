...
class OandaAPI():
    ...
    def fetch_candles(self, pair_name, count=20, granularity="M10"):
    ...
    def last_complete_candle(self,pair_name,granularity='M10'):
    ...
    def set_sl_tp(self,price,order_type,trade_id):
        ...
        data = {
            'order': {
                'timeInForce': 'GTC', 
                'price': str(price), #please note that it only accepts 5 decimal pts
                'type':order_type,
    ...
    def place_trade(self,pair,units,take_profit=None, stop_loss=None):
        ...
        data = {
            'order': {
                'units': units, #we use + and - to indicate a long or short order
                'instrument':pair,
                'timeInForce':'FOK',
                'type':'MARKET',
    ...
    def open_trades(self):
        ...
        if 'trades' not in data:  #this means we dont have any open trades but the request is working
            return [], True
...
#             if candle['complete'] == False: ####sometime it takes a long time for candle to 'complete, so in order to get the latest info, we skip this 'complete' criteria
#                 continue
            new_dict = {}
